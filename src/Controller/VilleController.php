...
class VilleController extends AbstractController
{
    ...
    /**
     * @Route("/", name="ville_name", methods={"POST"})
     */
    public function recherche(Request $request, VilleRepository $villeRepository): Response
    {
...
//        $offset = ($page - 1) * $limit;
        if ($request->get('search-txt') != '') {
            $allVilles = $villeRepository->findVilleByNom($request->get('search-txt'));
//dd($request->get('search-txt'));
//            dd($allVilles);
        } else {
//            $allVilles = $villeRepository->findAll($limit, $offset);
     ...
     * @Route("/{id}", name="app_ville_show", methods={"GET"})
     */
    public function showbyname(Ville $ville, name $name): Response
    {
        return $this->render('ville/show.html.twig', [
            'ville' => $ville,
        ]);
